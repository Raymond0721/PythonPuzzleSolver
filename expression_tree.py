    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self._root == OP_ADD:
            ...
            for subtree in self._subtrees:
                plus_result += subtree.eval(lookup)
            return plus_result
        else:
            multiplication_result = 1
            for subtree in self._subtrees:
                multiplication_result *= subtree.eval(lookup)
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        >>> exp_t = ExprTree('*',[ExprTree('a', []), \
                                 ExprTree('*', [ExprTree('a', []),\
                                                ExprTree(1, [])])])
        >>> look_up2 = {}
        >>> exp_t.populate_lookup(look_up2)
        >>> look_up2['a'] == 0
        True
        >>> len(look_up2) == 1
        True
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    for i in range(1, len(values)):
        ...
        for value in values[i]:
            construct_tree = ExprTree(value, [])
            curr_item.append(construct_tree)
            if value in OPERATORS:
                q.enqueue(construct_tree)
