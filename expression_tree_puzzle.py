...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        for variable in self.variables:
            if not 1 <= self.variables[variable] <= 9:
                return False
        ...
        for variable in self.variables:
            if self.variables[variable] == 0:
                for n in range(1, 10):
                    ...
                    new_tree = ExpressionTreePuzzle(tree_copy,
                                                    self.target)
                    new_tree.variables = self.variables.copy()
                    new_tree.variables[variable] = n
    ...
    def fail_fast(self) -> bool:
        ...
        """
        min_value = self.variables.copy()
        max_value = self.variables.copy()

        for variable in self.variables:
            if self.variables[variable] == 0:
                min_value[variable] = 1
                max_value[variable] = 9

        smallest_target = self._tree.eval(min_value)
        biggest_target = self._tree.eval(max_value)

        if (smallest_target > self.target) or (self.target > biggest_target):
            return True
