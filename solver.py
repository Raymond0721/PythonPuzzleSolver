...
class DfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        <seen> is either None (default) or a set of puzzle states' string
        representation+s, whose puzzle states can't be any part of the path to
        the solution.
        """

        seen = seen if seen else set()  # if not seen: set()
        if puzzle.__str__() in seen or puzzle.fail_fast():
            return []
        if puzzle.is_solved():
            return [puzzle]
        seen.add(puzzle.__str__())  # add it to the seen
        puzzle_extensions = puzzle.extensions()
        for state in puzzle_extensions:
            result = self.solve(state, seen)
            if result:
...
class BfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        while not q.is_empty():
            single_state = q.dequeue()
            last_state = single_state[-1]
            if not last_state.__str__() in seen and not last_state.fail_fast():
                seen.add(last_state.__str__())
                if last_state.is_solved():
                    return single_state
                for state in last_state.extensions():
                    q.enqueue(single_state + [state])
        return []
        #     single_state = q.dequeue()
        #     last_state = single_state[-1]
        #     if str(last_state) not in seen and not last_state.fail_fast():
        #         seen.add(str(last_state))
        #         if last_state.is_solved():
        #             return single_state
        #         for state in last_state.extensions():
        #             q.enqueue(single_state + [state])
