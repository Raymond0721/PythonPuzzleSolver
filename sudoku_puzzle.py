...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for r in range(self._n):
            for c in range(self._n):
                ...
                #     current_set = (self._row_set(r)
                #                    | self._column_set(c)
                #                    | self._subsquare_set(r, c))
                current_set = (self._row_set(r)
                               | self._column_set(c)
                               | self._subsquare_set(r, c)) if \
                    self._grid[r][c] == EMPTY_CELL else set()
                if EMPTY_CELL in current_set:
                    current_set.remove(EMPTY_CELL)
                if current_set == self._symbol_set:
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        seen = set()
        solutions = bfs_solver.solve(self)
        if not solutions:
            return False
        seen.add(str(solutions[-1]))
        check_unique = bfs_solver.solve(self, seen)
        if not check_unique:
            return True

        return False
        # use seen if the first method as seen, can we find the second method.
